...
function init() {
  ...
  if (!host) { transporter = null; return; }
...
export function mailEnabled(): boolean { init(); return !!transporter; }

export function appUrl(): string {
  return (process.env.APP_PUBLIC_URL || (process.env.CLIENT_ORIGIN || '').split(',')[0] || '').replace(/\/+$/, '');
}

export function brandEmail(heading: string, bodyHtml: string, cta?: { text: string; url: string }): string {
...
export async function sendMail(to: string | undefined | null, subject: string, html: string): Promise<boolean> {
  init();
