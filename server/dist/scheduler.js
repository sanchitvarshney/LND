...
const DAY = 86400000;
async function alreadyNotified(userId, type, title) {
    const since = new Date(Date.now() - 20 * 3600 * 1000);
    const n = await db_js_1.prisma.notification.findFirst({ where: { userId, type, title, createdAt: { gt: since } } });
    return !!n;
}
async function notify(userId, type, title, body) {
    if (await alreadyNotified(userId, type, title))
        return false;
    await db_js_1.prisma.notification.create({ data: { userId, type, title, body } });
    return true;
}
async function runReminders() {
    ...
    for (const a of assignments) {
        ...
        if (overdue && a.status !== 'overdue')
            await db_js_1.prisma.assignment.update({ where: { id: a.id }, data: { status: 'overdue' } });
        if (!overdue && !dueSoon)
            continue;
        ...
        if (fresh) {
            await (0, mailer_js_1.sendMail)(a.user.email, `${title}: ${a.module.title}`, (0, mailer_js_1.brandEmail)(title, `Your training <b>${a.module.title}</b> is ${overdue ? 'now <b>overdue</b> (was due ' + when + ')' : 'due on <b>' + when + '</b>'}. Please complete it to stay compliant.`, url ? { text: 'Go to my training', url } : undefined));
        }
    }
    // --- Certificate expiry / recertification ---
    const certs = await db_js_1.prisma.certificate.findMany({
    ...
    for (const c of certs) {
        ...
        if (expired) {
            ...
            await db_js_1.prisma.assignment.updateMany({ where: { userId: c.userId, moduleId: c.moduleId }, data: { status: 'assigned', completedAt: null } });
            ...
            if (fresh)
                await (0, mailer_js_1.sendMail)(c.user.email, `Recertification required: ${c.module.title}`, (0, mailer_js_1.brandEmail)('Recertification required', `Your certification for <b>${c.module.title}</b> has <b>expired</b>. Please retake the training to remain compliant.`, url ? { text: 'Start recertification', url } : undefined));
        }
        else if (expSoon) {
            ...
            if (fresh)
                await (0, mailer_js_1.sendMail)(c.user.email, `Certification expiring soon: ${c.module.title}`, (0, mailer_js_1.brandEmail)('Certification expiring soon', `Your certification for <b>${c.module.title}</b> expires on <b>${when}</b>. Plan to recertify before then.`, url ? { text: 'View training', url } : undefined));
        }
    }
}
let started = false;
...
function startScheduler() {
    ...
    const tick = async () => {
        ...
        try {
            await runReminders();
            console.log('[scheduler] reminders run for', day);
        }
        catch (e) {
