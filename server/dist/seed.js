...
exports.seed = seed;
exports.bootstrapFirstAdmin = bootstrapFirstAdmin;
...
async function seed() {
    ...
    for (const m of ['quizResponse', 'quizAttempt', 'userProgress', 'certificate', 'notification', 'assignment', 'question', 'assessment', 'video', 'trainingModule', 'auditLog', 'user']) {
        await db_js_1.prisma[m].deleteMany();
    }
    const hash = (pw) => bcryptjs_1.default.hashSync(pw, 10);
    const admin = await db_js_1.prisma.user.create({ data: { email: 'admin@acme.com', passwordHash: hash('Admin@123'), fullName: 'Avery Admin', role: 'admin', department: 'L&D' } });
    const manager = await db_js_1.prisma.user.create({ data: { email: 'manager@acme.com', passwordHash: hash('Manager@123'), fullName: 'Morgan Manager', role: 'manager', department: 'Engineering' } });
    const learner = await db_js_1.prisma.user.create({ data: { email: 'learner@acme.com', passwordHash: hash('Learner@123'), fullName: 'Lee Learner', role: 'learner', department: 'Engineering', managerId: manager.id } });
    const learner2 = await db_js_1.prisma.user.create({ data: { email: 'sam@acme.com', passwordHash: hash('Learner@123'), fullName: 'Sam Rivera', role: 'learner', department: 'Engineering', managerId: manager.id } });
    const mod1 = await db_js_1.prisma.trainingModule.create({ data: { title: 'Information Security Awareness', description: 'Core security hygiene every employee must know: phishing, passwords, data handling, and incident reporting. Required annually for compliance.', category: 'Security & Compliance', isMandatory: true, passThreshold: 70, validityDays: 365, status: 'published', createdById: admin.id } });
    await db_js_1.prisma.video.create({ data: { moduleId: mod1.id, title: 'Security Fundamentals', orderIndex: 0, durationSeconds: 15, sourceUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4', binSizeSeconds: 1 } });
    const assess1 = await db_js_1.prisma.assessment.create({ data: { moduleId: mod1.id, title: 'Security Awareness Knowledge Check', passingScore: 70, maxAttempts: 3, showAnswers: true } });
    await db_js_1.prisma.question.createMany({ data: [
    ...
    const mod2 = await db_js_1.prisma.trainingModule.create({ data: { title: 'Code of Conduct & Ethics', description: 'Company values, conflicts of interest, anti-bribery, and how to raise concerns. Mandatory for all staff.', category: 'Compliance', isMandatory: true, passThreshold: 80, validityDays: 365, status: 'published', createdById: admin.id } });
    await db_js_1.prisma.video.create({ data: { moduleId: mod2.id, title: 'Our Code of Conduct', orderIndex: 0, durationSeconds: 15, sourceUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4', binSizeSeconds: 1 } });
    const assess2 = await db_js_1.prisma.assessment.create({ data: { moduleId: mod2.id, title: 'Code of Conduct Quiz', passingScore: 80, maxAttempts: 3, showAnswers: true } });
    await db_js_1.prisma.question.createMany({ data: [
    ...
    for (const u of [learner, learner2]) {
        await db_js_1.prisma.assignment.create({ data: { userId: u.id, moduleId: mod1.id, assignedById: admin.id, dueAt: dueSoon, status: 'assigned' } });
        await db_js_1.prisma.assignment.create({ data: { userId: u.id, moduleId: mod2.id, assignedById: admin.id, dueAt: due, status: 'assigned' } });
    }
    await db_js_1.prisma.notification.create({ data: { userId: learner.id, type: 'assignment', title: 'New training assigned', body: 'Information Security Awareness is due in 2 days.' } });
    await db_js_1.prisma.auditLog.create({ data: { actorId: admin.id, action: 'seed.bootstrap', entityType: 'system', metaJson: JSON.stringify({ modules: 2 }) } });
...
async function bootstrapFirstAdmin() {
    ...
    await db_js_1.prisma.user.create({ data: {
            email: email.toLowerCase(), passwordHash: bcryptjs_1.default.hashSync(password, 10),
